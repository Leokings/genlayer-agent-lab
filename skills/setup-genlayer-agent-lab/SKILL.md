...
For MCP, check the installed host's help/schema, then merge the supplied server definition into its native configuration. For example, the Lab's generic `mcpServers["genlayer-lab"]` entry maps to current OpenClaw's `mcp.servers["genlayer-lab"]`; other hosts use their own format. Retain the supplied absolute command, arguments, and `LAB_MODE`, `LAB_ROLE`, `LAB_URL`, `LAB_RUN_ID`, and run-scoped `LAB_TOKEN`. Studio project runs use `LAB_MODE=workflow`; quick GLSim runs use `LAB_MODE=scenario` and a different tool interface. Do not change modes on an existing run or reuse another run's credentials. Every new run requires its generated scoped prompt. The bridge is stdio; its `LAB_URL` is the Lab HTTP API, not a remote MCP transport endpoint. Preserve unrelated settings and the user's chosen model.
...
After connecting to Studio, read `builtin_operations` in the observation for each permitted built-in's argument schema, example and decision-ID requirements. Use the declared fields exactly: for example, `read_evidence` uses `id`, not `evidence_id`. Quick mode uses `observe`, `request_decision`, `read_decision`, `act` and `finish`; it has no appeal tool. Follow that run's public task and permissions, preserve retry keys for identical requests, and use a new key when correcting arguments. Finish the run when the requested work is complete. Report observed actions and the run identifier without claiming access to the developer's private grade. If the run has expired, tell the user to provide a fresh reviewed run's settings rather than silently creating a replacement with administrator access.

## Use one dashboard

The default setup prepares the supported project Studio environment and opens the unified dashboard. Before a test, choose **Studio execution** (default) or **Quick simulation · GLSim**. Each mode has its own supported scenarios and connection tools; project-v2 JSON stays Studio-only and must not be converted blindly. Quick mode exposes 15 supplied non-appeal scenarios with a controlled consumer timeline and simulated application effects. Its statuses do not prove native GenLayer finality, and it does not simulate protocol appeals. Legacy binding import, `service_release` and the earlier Studio CLI remain advanced paths. The old developer-dashboard URL redirects to the unified dashboard, whose history preserves actual backend labels and earlier reports, including Studio and fixture runs.
...
Use the unified dashboard and choose the mode first. In **Studio execution**, use **Choose a test**, **Review test**, **Create test & connect agent**, then **Check connection**. Studio templates include prediction finalization, changed or upheld appeals, message delivery or repair, and evidence investigation. Custom project-v2 definitions use **Use my own contract** and `docs/SCENARIO_AUTHORING.md`. In **Quick simulation · GLSim**, select a supported escrow, treasury or generic scenario, **Review quick test**, confirm its controlled conditions, then **Create quick test**. Quick choices are normal, provisional, timeout, wrong scope and duplicate acknowledgement; appeal/revised scenarios are not offered. Already imported compatible legacy bindings use isolated `container-glsim`, not project import or conversion.
...
Dashboard Studio project runs have a 60-minute setup/connection allowance. Their selected behavioral duration begins only once Studio is ready and an authenticated agent observation has arrived; an early observation waits for readiness. Administrator inspections and connector discovery do not start it. Existing project CLI/API callers start immediately unless they opt into `wait_for_agent`. Bundled quick runs instead have ten minutes beginning after runtime preparation, without a first-observation allowance; prepare the agent host before creating one and connect promptly. Select the actual connection method and agent location, then use **Copy agent setup prompt** as the primary handoff. Give the tested agent only that new run's credential, ID, Lab URL and mode-specific adapter settings. Preserve `LAB_MODE=workflow` for Studio or `LAB_MODE=scenario` for quick, plus `LAB_ROLE=agent`, `LAB_RUN_ID`, `LAB_TOKEN` and `LAB_URL`. Use an absolute installed executable path when required. Keep administrator tools in a separate developer connection.
...
The developer's own agent should observe its task, use the declared operations and permissions, and finish. Open the readable report from the shared history and check its actual backend and evidence note. Studio project results can establish observed local execution and finality; quick GLSim results cover contract evaluation and the scripted consumer timeline, not native appeals or protocol finality. Earlier basic Studio scenarios can also have scripted timelines: their backend label alone does not prove a protocol appeal. Preserve the original limits of older Studio and fixture reports. Inspect the actions and passed or failed expected checks; completed lifecycle status is not a passing grade. A failed agent evaluation can be a valid integration result; an inconclusive runtime failure needs diagnosis before judging the agent.

For a Studio installation-only diagnostic, run one scripted control against the active Lab:
