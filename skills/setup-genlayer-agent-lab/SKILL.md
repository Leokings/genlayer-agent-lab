...
For OpenClaw, restart the owning Gateway after merging its MCP configuration, then use a fresh turn in the same agent. If restarting from inside that agent would interrupt the work, give the user that single terminal step. Use native tool discovery and the actual returned tool names; do not repeatedly spawn children or install an MCP Apps bridge to address a missing native connector. Keep the selected model and existing permissions. See `docs/OPENCLAW_QUICKSTART.md`.
