...
On a desktop, setup opens the browser automatically. On a VPS, use **Open my dashboard** on the [public setup page](https://genlayer-agent-lab-setup.vercel.app/setup.html#open-dashboard). It guides you through your existing Termius connection, one step at a time. An agent on the VPS cannot operate Termius on your laptop.

In the dashboard, choose **Connect this browser** and paste **Copy sign-in request** into your setup agent. Return to the browser after approval; no workspace key needs copying. This only opens your owner workspace. You still review a test before connecting the agent you want to evaluate. See [advanced connection and approval commands](VPS_QUICKSTART.md#advanced-terminal-connection-and-approval) only if you prefer the terminal.
...
Use **Copy setup prompt** on the [public setup page](https://genlayer-agent-lab-setup.vercel.app/setup.html), or paste the text below into your existing terminal-capable agent. It links to the [setup skill](../skills/setup-genlayer-agent-lab/SKILL.md) and authorizes necessary dependency installation:
